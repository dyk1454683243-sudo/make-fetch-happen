...
const getMetadata = (request, response, options) => {
  ...
  // HEAD is stored as metadata only, so keep the method and status even for
  // 200s. otherwise only save the status if it's not a 200 or 304
...
class CacheEntry {
  ...
  static async find (request, options) {
    try {
      ...
      var matches = await cacache.index.compact(options.cachePath, cacheKey(request), (A, B) => {
        // metadata-only HEAD 200s must not replace a GET body for the same url
        if ((A.integrity == null) !== (B.integrity == null)) {
          return false
        }
    ...
    for (const entry of matches) {
      ...
      if (_entry.policy.satisfies(request)) {
        // HEAD 200 is stored as metadata only (null integrity) and must not
        // be reused as a GET body. HEAD redirects may still satisfy GET.
        const meta = entry.metadata || {}
        const headOnly = meta.method === 'HEAD' ||
          (entry.integrity === null && meta.status === 200)
        const redirect = [301, 308].includes(meta.status)
        if (request.method === 'GET' && headOnly && !redirect) {
          continue
        }
  ...
  async store (status) {
    ...
    const cacheOpts = {
      ...
      integrityEmitter: this.response.body &&
        this.response.body.hasIntegrityEmitter && this.response.body,
    }

    let body = null
    // GET 200 responses stream their body into the cache, including empty
    // bodies. HEAD and redirects are stored as metadata only so a HEAD 200
    // cannot later be reused as an empty GET body.
    const storeBody = this.request.method === 'GET' && this.response.status === 200
    if (storeBody) {
      ...
      const onResume = () => {
        ...
        // cacache.put.stream rejects empty bodies with ENODATA. write an
        // explicit empty entry so GET 200s with no content are still cached.
        // TODO if the cache write fails, log a warning but return the response anyway
        // eslint-disable-next-line promise/catch-or-return
        cacheStream.promise().then(cacheWriteResolve, (err) => {
          if (err && err.code === 'ENODATA') {
            return cacache.put(this.options.cachePath, this.key, Buffer.alloc(0), {
              algorithms: this.options.algorithms,
              metadata: cacheOpts.metadata,
              size: 0,
              integrity: this.options.integrity,
