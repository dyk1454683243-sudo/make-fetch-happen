...
const remoteFetch = (request, options) => {
  ...
  return promiseRetry(async (retryHandler, attemptNum) => {
    ...
    try {
      ...
      // retryHandler throws EPROMISERETRY so promiseRetry can retry 5xx
      // responses. Re-throw those, and never retry POST.
      const isPromiseRetryResponse = err.retried instanceof fetch.Response
      if (req.method === 'POST' || isPromiseRetryResponse) {
        throw err
      }

      // Timeouts can have both a RETRY_ERRORS code (e.g. ETIMEDOUT) and
      // type request-timeout. Treat either as retryable so the rejection
      // is handled by retryHandler. The previous `code && type` check
      // treated that combination as a terminal throw (issue #315).
      const isRetryableFetchError = RETRY_ERRORS.includes(code) ||
        RETRY_TYPES.includes(err.type)
      ...
      // Unknown errors also retry, matching prior default-retry behavior.
